"""Kubernetes/Docker 健康检查探针与依赖健康仪表盘。

提供三个端点：
- /health/live:   存活探针（liveness）
- /health/ready:  就绪探针（readiness），检查 PG/Redis/Qdrant
- /health/dependencies: 完整依赖健康面板，含 LLM 断路器状态和验证反馈统计
"""
...
def _llm_provider_status() -> dict[str, dict[str, object]]:
    ...
    """
    try:
        from deerflow.llm_gateway import llm_gateway
    except Exception:
        return {}
...
def _feedback_stats() -> dict[str, object]:
    ...
    try:
        from deerflow.feedback import analyze_recent_failures
