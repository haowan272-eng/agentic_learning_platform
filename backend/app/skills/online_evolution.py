...
SideQuery = Callable[[str, str], str]
ConfirmWrite = Callable[[str], bool]
...
def extract_online_skill_candidate(
    *,
    ...
    system = (
        "You are Agentic Learning RAG's online Skill Extractor.\n"
        "Extract at most ONE reusable skill candidate from a conversation window.\n"
        "Output ONLY strict JSON: {\"skills\": []} or {\"skills\": [{...}]}.\n\n"
        "Candidate fields: name, description, when_to_use, instructions, evidence, tags.\n\n"
        "Rules:\n"
        "- USER turns are primary evidence; assistant turns are context only.\n"
        "- Extract durable workflow, output policy, implementation preference, correction, or repeated constraint.\n"
        "- Do not extract one-off task content, secrets, project facts, URLs, account IDs, exact dates, or assistant-only guesses.\n"
    ...
    parsed = _parse_json_object(side_query(system, json.dumps(payload, ensure_ascii=False)))
...
def maintain_online_skill_candidate(
    *,
    ...
    from .registry import create_skill, discover_skills, evolve_skill, retrieve_relevant_skills
    ...
    system = (
        "You are Agentic Learning RAG's online Skill Set Manager.\n"
        "Decide whether a candidate should add a new skill, merge into an existing skill, or be discarded.\n"
        "Output ONLY strict JSON.\n\n"
        "Schema: {\"action\":\"add|merge|discard\",\"target_skill\":\"existing name for merge\","
        "\"reason\":\"short reason\",\"merged_description\":\"optional\","
        "\"merged_when_to_use\":\"optional\",\"merged_instructions\":\"optional full SKILL.md body\"}\n\n"
        "Rules:\n"
        "- Prefer merge over add when the same capability already exists.\n"
        "- Discard if the candidate duplicates an existing skill and adds no durable improvement.\n"
        "- If merging, preserve useful existing guidance and add only durable new guidance.\n"
    ...
    decision = _parse_json_object(side_query(system, json.dumps(payload, ensure_ascii=False)))
    ...
    if confirm_write is not None and not confirm_write(write_summary):
        return {
            "ok": False,
...
def online_ingest(
    *,
    ...
    from .registry import record_online_provenance

    try:
        candidate = extract_online_skill_candidate(
            messages=messages,
    ...
    try:
        result = maintain_online_skill_candidate(
            candidate=candidate,
            side_query=side_query,
            retrieved_reference=retrieved_reference,
...
def judge_retrieved_skill_usage(
    *,
