...
def generated_skills_root() -> Path:
    raw = os.getenv("SKILL_GENERATED_DIR", "").strip()
    return Path(raw) if raw else project_root() / "skills" / "generated"


def builtin_skills_root() -> Path:
    raw = os.getenv("SKILL_BUILTIN_DIR", "").strip()
    return Path(raw) if raw else project_root() / "skills" / "builtin"


def user_skills_root() -> Path:
    raw = os.getenv("SKILL_USER_DIR", "").strip()
    return Path(raw) if raw else Path.home() / ".agentic_learning_rag" / "skills"


def get_evolution_dir() -> Path:
    raw = os.getenv("SKILL_EVOLUTION_DIR", "").strip()
    return Path(raw) if raw else project_root() / ".agentic_learning_rag" / "skill-evolution"
...
def _safe_skill_slug(name: str) -> str:
    ...
    return f"skill-{digest}" if raw else "unknown"
    ...
    *,
    ...
    for item in list(messages or [])[-max_messages:]:
        ...
        if role not in {"user", "assistant"} or not content:
            continue
        compacted.append({"role": role, "content": _preview(content, max_chars)})
    return compacted
...
def record_skill_feedback(*, skill_name: str, rating: str, note: str = "") -> None:
    _append_jsonl(
        get_evolution_dir() / USAGE_LOG,
        {
            "event": "feedback",
...
def resolve_skill_file(
    ...
    *,
    ...
    if normalized == "active" and active_dir:
        skill_file = Path(active_dir) / "SKILL.md"
        if skill_file.is_file():
            return skill_file

    search_roots: list[Path] = []
    if normalized in {"user", "active"}:
        search_roots.append(user_skills_root())
    if normalized in {"generated", "project", "active"}:
        search_roots.append(generated_skills_root())
    if normalized in {"builtin", "project", "active"}:
        search_roots.append(builtin_skills_root())

    for root in search_roots:
        found = _find_skill_file_by_name(root, skill_name)
        if found:
            return found
    return None
...
def _skills_root(target: str) -> Path:
    normalized = (target or "generated").strip().lower()
    if normalized == "user":
        return user_skills_root()
    if normalized == "builtin":
        return builtin_skills_root()
    return generated_skills_root()
...
def create_skill_file(
    *,
    ...
    if tags:
        ...
        if normalized_tags:
            meta["tags"] = ",".join(normalized_tags[:12])
    tools = _normalize_allowed_tools(allowed_tools)
    if tools:
        meta["allowed-tools"] = tools

    skill_dir.mkdir(parents=True, exist_ok=False)
    skill_file.write_text(format_frontmatter(meta, _skill_body(instructions, evidence)), encoding="utf-8")
...
def evolve_skill_file(
    *,
    ...
    meta: dict[str, object] = dict(parsed.meta)
    resolved_name = str(meta.get("name") or skill_file.parent.name)
    previous_version = str(meta.get("version") or "0.1.0")
    ...
    meta["version"] = _bump_patch(previous_version)
    meta["last-evolved"] = _utc_now()
    meta["evolution-count"] = str(_parse_int(str(meta.get("evolution-count") or ""), 0) + 1)
    ...
    if tags:
        existing_tags = [
            part.strip()
            for part in str(meta.get("tags") or "").split(",")
            if part.strip()
        ]
        for tag in tags:
        ...
        if existing_tags:
            meta["tags"] = ",".join(existing_tags[:12])
...
def _maybe_prune_stale_skill(skill_name: str, stats: dict[str, Any]) -> bool:
    min_retrieved = _parse_int(os.getenv("SKILL_USAGE_PRUNE_MIN_RETRIEVED"), 40)
    max_used = _parse_int(os.getenv("SKILL_USAGE_PRUNE_MAX_USED"), 0)
    ...
    if not skill_dir.is_dir() or skill_dir.name.startswith("."):
        return False
    try:
        skill_dir.relative_to(generated_skills_root())
    except ValueError:
        return False
...
def load_skill_stats() -> dict[str, dict[str, Any]]:
    ...
    if usage_path.is_file():
        for line in usage_path.read_text(encoding="utf-8", errors="replace").splitlines():
            ...
            item = stats.setdefault(
                skill,
                {"created": 0, "invocations": 0, "feedback": 0, "evolutions": 0},
            )
            event = row.get("event")
            if event == "create":
