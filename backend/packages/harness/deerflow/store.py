...
from .scenarios import get_scenario, scenario_for_task_type
...
class AgentRuntimeStore:
    ...
    def create_task(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if scenario is None:
            scenario_key, scenario = scenario_for_task_type(payload.get("task_type"))
        scenario_inputs = payload.get("scenario_inputs") or {}
        initial_state = {"source_policy": payload.get("source_policy") or "auto"}
        if scenario_key and scenario:
            initial_state = {
        ...
        task = AgentTask(
            ...
            state_json=_json_dump(initial_state),
    ...
    def save_tool_call(self, payload: dict[str, Any]) -> None:
        ...
        with SessionLocal() as db:
            row = None
            if idempotency_key:
                row = db.query(AgentToolCall).filter(AgentToolCall.idempotency_key == idempotency_key).first()
            if row is None:
                row = AgentToolCall(
                    task_id=payload["task_id"],
                ...
                db.add(row)
            row.input_json = _json_dump(payload.get("input") or {})
            row.output_json = _json_dump(payload.get("output", payload.get("result") or {}))
            row.result_json = _json_dump(payload.get("result", payload.get("output") or {}))
            row.ok = 1 if payload.get("ok", True) else 0
            ...
            row.prompt_tokens = payload.get("prompt_tokens")
            row.completion_tokens = payload.get("completion_tokens")
