"""Verifier Feedback Loop — collect, analyze, and act on verification failures.

The loop has three stages:

1. **Collect** — structured failure records are persisted via the memory event
   system whenever the Verifier rejects or flags a proposal.
...
@dataclass
class FailureSignal:
    """A single Verifier-detected failure, stored for later analysis."""
